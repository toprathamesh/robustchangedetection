...
class RobustChangeDetector:
    ...
    def detect_changes(self, before: np.ndarray, after: np.ndarray) -> tuple:
        ...
        if len(before.shape) == 3:
            ...
            weights = [0.3, 0.5, 0.2] if before.shape[2] >= 3 else [1.0/before.shape[2]] * before.shape[2]
            combined_diff = np.zeros_like(differences[0])
            for i, diff in enumerate(differences):
                combined_diff += diff * weights[min(i, len(weights)-1)]
        else:
            # Grayscale processing
            combined_diff = np.abs(before.astype(float) - after.astype(float)) / 255.0
    ...
    def _calculate_statistics(self, change_map: np.ndarray, before: np.ndarray, after: np.ndarray) -> dict:
        ...
        if len(before.shape) == 3 and changed_pixels > 0:
            ...
            avg_color_change = np.mean(after_changed.astype(float) - before_changed.astype(float), axis=0)
...
def create_comprehensive_report(scenarios, results):
    ...
    fig.suptitle('🛰️ ROBUST CHANGE DETECTION SYSTEM - COMPREHENSIVE ANALYSIS REPORT', 
    ...
    for scenario_name, scenario_data in scenarios.items():
        ...
        plt.title(f'Before\n{scenario_data["description"].split(" - ")[0]}', fontsize=10)
        plt.axis('off')
        
        # After image
        ax = plt.subplot(3, 6, row*6 + 2)
        plt.imshow(scenario_data['after'])
        plt.title(f'After\n{scenario_data["description"].split(" - ")[1]}', fontsize=10)
        plt.axis('off')
        
        # Change map
        ax = plt.subplot(3, 6, row*6 + 3)
        plt.imshow(change_map, cmap='Reds')
        plt.title(f'Change Map\n{stats["change_percentage"]:.1f}% Changed', fontsize=10)
        plt.axis('off')
...
"""
    ...
    plt.tight_layout()
    plt.savefig('test_results/comprehensive_analysis_report.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    print(f"🔧 Initialized detector (threshold: {detector.threshold}, min_area: {detector.min_change_area})")
    ...
    for scenario_name, scenario_data in scenarios.items():
        change_map, confidence, stats, proc_time = run_analysis(detector, scenario_name, scenario_data)
        results[scenario_name] = (change_map, confidence, stats, proc_time)
        total_processing_time += proc_time
        
        # Save individual scenario results
        Image.fromarray(scenario_data['before']).save(f'test_results/{scenario_name}_before.png')
        Image.fromarray(scenario_data['after']).save(f'test_results/{scenario_name}_after.png')
    ...
    print(f"   📈 Average processing speed: {total_processing_time/len(scenarios):.3f}s per scenario")
    
    avg_confidence = np.mean([results[s][1] for s in results])
    avg_change_pct = np.mean([results[s][2]['change_percentage'] for s in results])
